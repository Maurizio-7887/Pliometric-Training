...
export const GuidedTimer: React.FC<Props> = ({ workout, onExit, onStart, onComplete }) => {
  ...
  const start = () => {
    ...
    if (firstStart) {
      ...
      if (isSpotifyLoggedIn()) playSpotifyLink(getSpotifyLink());
    } else if (isSpotifyLoggedIn()) resumeSpotifyPlayback();
  ...
  const reset = () => { setRunning(false); setIdx(0); setSetNo(1); setPhase('ready'); setLeft(5); deadline.current = 0; spokenSecond.current = -1; window.speechSynthesis?.cancel(); };
  ...
  return <div className="guided-timer min-h-screen flex flex-col gap-4 pb-5">
    ...
    <div className={`card ${phase === 'work' ? 'bg-primary text-primary-content' : phase === 'rest' ? 'bg-secondary text-secondary-content' : 'bg-base-200'} grow`}><div className="card-body p-5 items-center text-center justify-between"><div><span className="badge badge-lg">{phase === 'ready' ? 'CONTO ALLA ROVESCIA' : phase === 'work' ? 'LAVORO' : 'RECUPERO'}</span><h2 className="text-2xl font-bold mt-3">{exercise.name}</h2><p className="opacity-80">Serie {setNo} di {exercise.sets} · {exercise.prescription}</p></div><MovementAnimation kind={exercise.kind} active={running} id={exercise.id} /><div><div className="font-mono text-7xl font-black tabular-nums">{Math.floor(left / 60)}:{String(left % 60).padStart(2, '0')}</div><progress className="progress w-full mt-3" value={pct} max="100" /><p className="mt-3 font-medium">{exercise.cues.join(' · ')}</p></div></div></div>
    <div className="grid grid-cols-4 gap-2"><button className="btn btn-ghost" onClick={reset}><RotateCcw /></button><button className="btn btn-primary col-span-2 btn-lg" onClick={() => running ? pause() : start()}>{running ? <><Pause /> PAUSA</> : <><Play /> {startedAt.current ? 'RIPRENDI' : 'START'}</>}</button><button className="btn btn-ghost" onClick={next}><SkipForward /></button></div>
