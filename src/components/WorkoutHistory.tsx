...
export const WorkoutHistory:React.FC<Props> = ({logs, onClear}) => <section className="workout-history">
 ...
 {logs.length===0 ? <div className="history-empty">Il primo allenamento comparirà qui appena viene avviato.</div> : <div className="history-list">{logs.slice(0,12).map(log=><article key={log.id} className="history-card">
  ...
  {log.runRepetitions?.length ? <div className="history-runs">
    <div className="history-run-summary"><span>Distanza GPS <b>{Math.round(log.totalDistanceMeters ?? 0)} m</b></span>{log.averagePaceSecondsPerKm!=null&&<span>Passo medio <b>{pace(log.averagePaceSecondsPerKm)}</b></span>}</div>
    <div className="history-repetitions">{log.runRepetitions.map(rep=><div key={rep.repetition}><span>Ripetuta {rep.repetition}</span><b>{pace(rep.paceSecondsPerKm)}</b></div>)}</div>
  </div> : null}
 </article>)}</div>}
