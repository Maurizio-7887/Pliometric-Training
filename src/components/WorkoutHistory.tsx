...
export const WorkoutHistory:React.FC<Props> = ({logs, onClear}) => <section className="card bg-base-200"><div className="card-body p-4 gap-3">
 ...
 {logs.length===0 ? <p className="text-sm text-base-content/60">Il primo allenamento comparirà qui appena viene avviato.</p> : <div className="space-y-2">{logs.slice(0,12).map(log=><div key={log.id} className="rounded-box bg-base-300 p-3 text-sm">
  <div className="flex justify-between gap-2"><strong>{log.workoutTitle}</strong><span className={`badge badge-sm ${statusClass(log.status)}`}>{statusLabel(log.status)}</span></div>
  ...
  {log.plannedSetCount != null && <p className="mt-2 text-xs text-base-content/70">Progresso pliometria: <b className="text-base-content">{log.completedSetCount ?? 0}/{log.plannedSetCount} serie</b>{log.plannedExerciseCount != null && <> · {log.completedExerciseCount ?? 0}/{log.plannedExerciseCount} esercizi</>}</p>}
